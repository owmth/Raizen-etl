...
xls = pd.ExcelFile('rawdata/vendas-combustiveis-m3.xls')
...
def write_df(formatado_df, nome):
    ...
    filename = '%s.csv' % nome
    print(filename)
    formatado_df.to_csv(filename, header=True, index=False)
    return formatado_df
